...
def main(args=None):
  ...
  STELENGTH = 13.0  # Shoulder to Elbow Length
  ETWLENGTH = 13.0  # Elbow to Wrist Length
  WTHLENGTH = 20.0  # Wrist to Hand Tip Length
  ...
  # 初期値
  shoulder_angle_deg = -90.0
  elbow_angle_deg = 0.0
  wrist_angle_deg = 0.0

  if STELENGTH + ETWLENGTH < z:
      print(0)

  elif 0 < z <= STELENGTH + ETWLENGTH:
      print(1)
      shoulder_angle_deg = -degrees(asin(z / (STELENGTH + ETWLENGTH)))
      elbow_angle_deg = shoulder_angle_deg - 90
      wrist_angle_deg = -shoulder_angle_deg

  elif z == 0:
      print(2)
      shoulder_angle_deg = 0
      elbow_angle_deg = -90
      wrist_angle_deg = 0
      ...
      print(3)
      shoulder_angle_deg = 0
      elbow_angle_deg = degrees(asin(z / ETWLENGTH))
      wrist_angle_deg = -90 - elbow_angle_deg

  # 足元の物体
  elif -25 < z <= -ETWLENGTH:
      print(4)
      shoulder_angle_deg = 0
      elbow_angle_deg = degrees(asin((z + WTHLENGTH) / ETWLENGTH)) - 90
      wrist_angle_deg = -elbow_angle_deg

  else:
    print(5)

  # ラジアンに変換
  shoulder_angle_rad = radians(shoulder_angle_deg)
  elbow_angle_rad = radians(elbow_angle_deg)
  wrist_angle_rad = radians(wrist_angle_deg)
  
  # 結果を度とラジアン両方で表示
  print(f"肩の角度（度）: {shoulder_angle_deg}°")
  print(f"肘の角度（度）: {elbow_angle_deg}°")
  print(f"手首の角度（度）: {wrist_angle_deg}°")
  print(f"肩の角度（ラジアン）: {shoulder_angle_rad} rad")
  print(f"肘の角度（ラジアン）: {elbow_angle_rad} rad")
  print(f"手首の角度（ラジアン）: {wrist_angle_rad} rad")

  node.moveArmRad([0.0, shoulder_angle_rad, elbow_angle_rad, 0.0, wrist_angle_rad, 0.0], 0.0, 3.0)
